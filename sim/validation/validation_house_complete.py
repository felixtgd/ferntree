...
def plot_results(df_annual, month):
    ...
    annual_grid_consumption = df_annual['P_total'][df_annual['P_total']>0].sum()/1000
    annual_grid_feed_in = df_annual['P_total'][df_annual['P_total']<0].sum()/1000
    annual_self_consumption = annual_baseload_demand + annual_heating_demand_el - annual_grid_consumption
    ...
    axs[1].plot(df.index, df["P_load_pred"], color="tab:grey", linestyle=":", label="P_load_pred")
    ...
    textstr = f"Baseload demand: {annual_baseload_demand:.2f} MWh/a\n" \
    ...
    props = dict(boxstyle='round', facecolor='white', alpha=0.5)
    axs[2].text(0.05, 0.95, textstr, transform=axs[2].transAxes, fontsize=10, verticalalignment='top', bbox=props)
    

    # Set title of figure with annual consumption, pv size, battery capacity and power
    axs[0].set_title(f"Annual consumption: {annual_consumption} kWh, PV: {pv_size} kWp, Battery: {bat_cap} kWh & {bat_pwr} kW")
