...
# Constraints
constraints = [x >= -p_max, x <= p_max, cp.cumsum(x) >= lb, cp.cumsum(x) <= ub]
...
Soc_bat_qp = np.append(
    bat_soc_init, bat_soc_init + np.cumsum(P_bat_qp)[:-1]
)  # this only works when timebase is 1h
...
def plot_power_profiles(start=0, end=7 * 24):
    ...
    ax1.plot(
        time_series[start:end],
        P_bat_valley[start:end],
        color="tab:green",
        label="Battery VF [kW]",
    )
    ax1.plot(
        time_series[start:end],
        P_total_valley[start:end],
        color="tab:purple",
        label="Total VF [kW]",
    )
    ...
    ax2.plot(
        time_series[start:end],
        Soc_bat_qp[start:end],
        color="tab:blue",
        linestyle=":",
        label="QP",
    )
    ax2.plot(
        time_series[start:end], Soc_bat_valley[start:end], color="tab:green", label="VF"
    )
    ...
    # Set title
    ax1.set_title(
        f"Z = {Z} kW, baseload: {annual_consumption} kWh, PV: {pv_size} kWp, Battery: {bat_cap} kWh & {bat_pwr} kW"
    )
...
for Z in Z_list:
    ...
    for t in range(n):
        ...
        P_bat_valley[t], soc_t = set_battery_power(
            Z, P_net_load[t], soc_t, bat_pwr, bat_cap
        )
...
def set_fill_levels(P_total):
    ...
    if True:
        Z_charge = (
            np.mean(P_total[P_total < 0]) if P_total[P_total < 0].size != 0 else -0.5
        )
        ...
        Z_charge = (
            np.percentile(P_total[P_total < 0], 75)
            if P_total[P_total < 0].size != 0
            else -0.5
        )
    ...
    if True:
        Z_discharge = (
            np.mean(P_total[P_total > 0]) if P_total[P_total > 0].size != 0 else 0.3
        )
        Z_discharge = 0.1 * Z_discharge  # reduce Z_discharge to 10% of mean value
    else:
        Z_discharge = (
            np.percentile(P_total[P_total > 0], 0.1)
            if P_total[P_total > 0].size != 0
            else 0.3
        )
...
for t in range(timesteps):
    ...
    Soc_bat_valley[t] = soc_t
    P_bat_valley[t], soc_t, Z_t = set_battery_power(
        Z_charge, Z_discharge, p_t, soc_t, bat_pwr, bat_cap
    )
    Z_cd[t] = Z_t
...
def plot_power_profiles_vf(start=0, end=7 * 24):
    ...
    ax1.plot(
        time_series[start:end], Z_cd[start:end], color="tab:grey", label="Z_cd [kW]"
    )
    ax1.plot(
        time_series[start:end],
        P_predictions[start:end],
        color="tab:pink",
        label="P_pred [kW]",
    )
    ...
    ax1.text(
        0.05,
        0.95,
        f"2-norm QP: {np.linalg.norm(P_total_qp):.2f} \
             \n2-norm VF: {np.linalg.norm(P_total_valley):.2f}",
    ...
    # Set title
    ax1.set_title(
        f"Z_charge mean = {np.mean(Z_charge):.2f} kW, Z_discharge = {np.mean(Z_discharge):.2f} kW, baseload: {annual_consumption} kWh, PV: {pv_size} kWp, Battery: {bat_cap} kWh & {bat_pwr} kW"
    )
    ...
    fig.savefig(
        os.path.join(
            script_dir,
            f"results/Z={np.mean(Z_charge):.2f}_{np.mean(Z_discharge):.2f}_power_profiles.png",
        )
    )
