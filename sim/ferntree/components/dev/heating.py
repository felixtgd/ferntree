from dev import device, model_2R3C, heating_dev
...
class HeatingSys(device.Device):
    ...
    def startup(self):
        ...
        - Initializes current state of the heating system
        """
        # Thermostat controller for heating system
        self.heating_ctrl = heating_ctrl.HeatingCtrl(self.host)
        self.heating_ctrl.temp_setpoint = self.temp_setpoint
        self.heating_ctrl.deadband = self.deadband
        self.heating_ctrl.P_heat_max = self.P_heat_max
        ...
        # Heating device
        self.heatingDev = heating_dev.HeatingDev(self.host)
        self.heatingDev.P_heat_max = self.P_heat_max
        self.heatingDev.cop = self.cop


        if self.heated_area is not None:
            self.P_hgain = (
                3 / 1e3 * self.heated_area
            )  # internal heat gains constant at 3 W/m2
    ...
    def timetick(self):
        ...
        T_in, T_en = self.thermal_model.compute_thermal_response(
            T_amb, P_solar, P_hgain, P_heat_th_next
        )

        # Calculate electricity demand of heating system
        P_heat_el = self.heatingDev.calc_electr_demand(P_heat_th_next)
        ...
        self.current_state["P_heat_el"] = P_heat_el
