...
class HeatingSys(device.Device):
    ...
    def timetick(self):
        ...
        # Thermal model: Compute thermal response of building to previous timestep
        T_in_next, T_en_next = self.thermal_model.compute_thermal_response(T_in, T_en, T_amb, P_solar, P_heat_th)

        # Heating controller: Determine control signal for heating device based on thermal response of building
        ctrl_signal = self.heating_ctrl.set_ctrl_signal(T_in_next)
