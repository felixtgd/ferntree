...
class BatteryCtrl(device.Device):
    ...
    def fill_level_battery_power(self, Z_charge, Z_discharge, p_t, soc_t, bat_max_pwr, bat_cap):
    ...
    def set_fill_levels(self, P_pred):
        ...
        Z_discharge = np.mean(P_pred[P_pred > 0]) if P_pred[P_pred > 0].size != 0 else 0.3
